...
// 3. 人脸模型必须打进 asar（运行时用 buffer 从 asar 内读取）
try {
    const entries = listPackage(path.join(RES, 'app.asar'));
    for (const m of ['det_10g.onnx', 'w600k_r50.onnx']) {
        if (entries.some(e => e.includes(`insightface/${m}`))) {
            console.log(`OK: asar 内含 models/insightface/${m}`);
        } else {
            console.log(`MISSING: asar 内未找到 models/insightface/${m}`);
            fail = true;
        }
    }
} catch (e) {
    console.log(`MISSING: 无法读取 app.asar（${e.message}）`);
    fail = true;
}
